...
function profilListesi(fenomenler) {
  const yeniDizi = fenomenler.slice();
  for (let key in yeniDizi) {
    yeniDizi[key] = yeniDizi[key]["profile"];
  }
  return yeniDizi;
}
...
function enFenomenler(fenomenler) {
  const enFenomen = [];
  for (let key in fenomenler) {
    if (fenomenler[key]["followers"] > 100000000) {
      enFenomen.push(fenomenler[key].profile);
    }
  }
  return enFenomen;
}
...
function fenomenGonderimSayisi(fenomenler, profile) {
  for (let key in fenomenler) {
    if (fenomenler[key]["profile"] === profile) {
      return fenomenler[key]["posts"];
    }
  }
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, platform) {
  ...
  let gonderi = "";
  for (let key in fenomenler) {
    if (fenomenler[key]["platform"] === platform) {
      if (fenomenler[key]["posts"] > max) {
        max = fenomenler[key]["posts"];
        gonderi = fenomenler[key]["profile"];
      }
    }
  }
  return gonderi;
